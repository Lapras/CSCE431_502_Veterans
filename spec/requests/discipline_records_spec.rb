...
RSpec.describe 'DisciplineRecords', type: :request do
  ...
  let!(:record1) { create(:discipline_record, user: user, given_by: user, points: 5) }
  let!(:record2) { create(:discipline_record, user: other_user, given_by: other_user, points: 3) }
  ...
  describe 'GET /discipline_records' do
    it 'returns a successful response' do
      ...
      expect(response.body).to include(record1.reason)
      expect(response.body).not_to include(record2.reason)
    end
  end
  ...
  describe 'GET /discipline_records/:id' do
    context 'when accessing own record' do
      it 'returns a successful response' do
        get discipline_record_path(record1)
        expect(response).to have_http_status(:ok)
        expect(response.body).to include(record1.reason)
      end
    end
    ...
    context "when accessing another user's record" do
      it 'redirects to root with an alert' do
        get discipline_record_path(record2)
