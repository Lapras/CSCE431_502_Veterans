...
class ApprovalsController < ApplicationController
  ...
  def index
    ...
    @pending_recurring = RecurringExcusal.pending.includes(:user).order(created_at: :desc)
    @approved_recurring = RecurringExcusal.approved.includes(:user,
                                                             :recurring_approval).order(updated_at: :desc).limit(20)
    @denied_recurring = RecurringExcusal.denied.includes(:user, :recurring_approval).order(updated_at: :desc).limit(20)
  end
